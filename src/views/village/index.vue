    ...
    <!-- <div class="city-title">
      ...
      <div class="mui-content mui-row">
        <div class="mui-col-xs-3">
          ...
          >
            <a id="a" @click="search(item.name)" v-for="(item,key) in village" :key="key" class="mui-control-item"  data-index="0" :class="{'mui-active':item.name==curProvince}">{{item.name}}</a>
          </div>
        </div>
        <div
        ...
        <div class="mui-loading" v-if="loading">
          ...
          <div  class="mui-control-content"  v-if="!loading">
            <ul class="mui-table-view">
              <li class="mui-table-view-cell" v-for="item in cityData" :key="item.adcode">{{item.name}}</li>
            </ul>
          </div>
        </div>
      </div> -->
